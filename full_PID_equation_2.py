...
class MoorebotLaneFollower(Node):
    def __init__(self, bot_name, save_debug=False):
        ...
        self.angular_gain = 0.0085 #oscillations: reduce Kp (Kd will help offset this)
        ...
        # integral parameters
        # self.ki (gain)
        # self.integral (error integral for accumulation)

        self.ki = 0.0001            # integral gain
        self.kd = 0.0015            # derivative gain
        self.integral = 0.0         # accumulated error integral
        self.max_integral = 200.0   # clamp limit
        ...
        try:
            ...
            if self.intersection_state == "FOLLOW_LANE":
                ...
                if lane_center is not None:
                    ...
                    # deadband, will use "I" term instead.
                    # "I" will sum the small (<8) error over time. after error accumulates to a 
                    # certain threshold, the output will push back to center.
                    # if abs(error) < 8:
                    #    error = 0


                    # P implementation (u = K * e)
                    p_term = self.angular_gain * error
