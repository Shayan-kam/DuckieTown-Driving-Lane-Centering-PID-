...
class MoorebotLaneFollower(Node):
    def __init__(self, bot_name, save_debug=False):
        ...
        # --- FULL PID GAINS AND STATE TRACKING ---
        self.kp = 0.0085
        self.ki = 0.0001
        self.kd = 0.0035

        self.integral_error = 0.0
        self.integral_max = 0.25      # anti-windup clamp limit
        self.prev_error = 0.0
    ...
    def reset_pid(self):
        """Clears accumulated integral and resets derivative baseline."""
        self.integral_error = 0.0
        self.prev_error = 0.0
        self.last_pid_time = None
    ...
    def process_image(self):
        ...
        try:
            ...
            white_cx = self.get_centroid(white_mask, min_area=40)
            ...
            if self.intersection_state == "FOLLOW_LANE":
                ...
                if lane_center is not None:
                    ...
                    error = float(img_center - lane_center)

                    if abs(error) < 8:
                        error = 0.0

                    # --- FULL DISCRETE PID CALCULATION ---
                    ...
                    # Line lost: reset PID state so error doesn't spike when reacquired
                    self.reset_pid()
